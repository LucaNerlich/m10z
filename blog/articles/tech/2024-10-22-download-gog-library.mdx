---
slug: download-gog-library
title: 'Sunday-Projects - Komplettes Backup der GoG Library'
date: 2024-10-22
authors: [luca]
tags: [artikel, tech, sunday-projects, luca]
draft: false
image: /img/tech/sundayprojects/gogbackup.png
---

import Image from '@theme/IdealImage'

'Sunday Projects' sind die kleinen, technischen Projekte und Spielereien mit denen ich in unregelmäßigen Abständen meine Zeit verbringe.

<Image img={require('/static/img/tech/sundayprojects/gogbackup.png')} />

<!--truncate-->

Wer spiele bei [Good Old Games](https://gog.com) kauft, hat stets Zugriff auf Offline-Installer saemtlicher Spiele in der Account-Bibliothek. GOG stellt zwar den Steam Konkurrent 'Galaxy' bereit, dieser ist aber nicht als Archiv / Backup Tool gedacht, sondern *nur* fuer den *day-today* Kram (Runterladen, Spielen, Shoppen).

Moechte man nun seine komplette  Bibliothek runterladen, bzw. die Offline-Installer sichern, stehen aktuell zwei Moeglichkeiten zur Verfuegung.

1. Der **lgogdownloader** von Sude-
    - https://github.com/Sude-/lgogdownloader
    - https://www.gog.com/forum/general/lgogdownloader_gogdownloader_for_linux
2. Das **gogrepoc** Python Script von Kalanyr
    - https://github.com/Kalanyr/gogrepoc
    - https://www.gog.com/forum/general/gogrepopy_python_script_for_regularly_backing_up_your_purchased_gog_collection_for_full_offline_e/page199

Der **lgogdownloader** muss selber unter Linux kompiliert werden, das Python Skript laeuft fast standalone.
Dieser Post ist entsprechend eine kurze Anleitung, wie man das Python Script **gogrepoc** bei sich, unter Windows 11, zum Laufen bekommen.

1. Python3 installieren - am einfachsten ueber den [Windows Store](https://www.microsoft.com/store/productId/9NCVDN91XZQP?ocid=pdpshare)
2. Das gogrepoc script herunterladen
    - https://github.com/Kalanyr/gogrepoc
    - Entweder per git, oder einfach [als zip](https://github.com/Kalanyr/gogrepoc/archive/refs/heads/master.zip)
3. Auf der Ziel-Festplatte einen neuen Ordner 'gog-backups', oder aehnlicher Name, erstellen und das Skript hier, entpackt, ablegen
4. Sicherstellen, dass Python3 auf der Kommandozeile verfuebgar ist
    - Im Terminal / CMD / Powershell `python3 -v` eingeben
    - Die etwaige Python REPL per `exit()` verlassen
5. Im Ordner des `gogrepoc.py` Skripts folgende Abhaengigkeiten installieren
    1. `pip3 install html5lib`
    2. `pip3 install pyOpenSSL`
6. Sollten weitere Fehler folgender Art auftreten, den `pip3 install <library>` Befehl anpassen und mit der fehlenden Abhaengigkeit erneut ausfuehren
    - `ModuleNotFoundError: No module named 'html5lib'`
7. Das Skript ausfuehren und einloggen
    - `python3 gogrepoc.py login`
8. Die GOG Bibliothek scannen und initial alle Metadaten herunterladen
    - `python3 gogrepoc.py update`
9. Kompletten Download starten
    - `python3 gogrepoc.py download -lang en de`
    - Das Skript kopiert erst ganz zum Schluss alle heruntergeladenen Dateien in die dedizierten Spieleordner


Danke fürs Lesen.

Meldet euch bei Fragen gerne auf [unserem Discord](https://discord.gg/bYPsrFxxtu).
