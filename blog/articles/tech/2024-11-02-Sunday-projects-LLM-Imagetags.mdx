...
Scheinbar ist es schon tradition das irgendwie ein paar Projekte sich mit AI ähnlichem beschäftigt, nachdem ich bereits Transktriptionen gemacht habe und @tilmobaxter seine Bilder mit AI generiert. Nun widme ich mich darum wie es mir sehr viel arbeit sparen kann und meine Bildersammlung an Qualität gewinnt.  
...
### Welches Modell?
Prinzipiell machen alle Modelle ähnliches und haben ein Vision Modell implementiert. Der Unterschied zwischen den Sprachmodellen ist die größe des Paramterraumes. Das ist insofern wichtig, weil nicht nur ein größeres Modell umfangreichere Antworten liefert, sondern auch sehr viel mehr Rechenleistung benötigt. Auf meinem Notebook konnte ich problemlos ein Modell mit 7 milliarden Paramtern (7B) ausführen. Naja, eigentlich gab es ein Problem: Bei Ollama gab es bis vor Kurzem das Problem, dass nach paar Prompts das Modell aufgehangen hat und nichts mehr weiter ging. Das ist mit der Version 0.4 anscheined gelöst und ich hatte bislang keine Probleme nach über 200 Bildern. Es gibt auch durchaus qualitative Unterschiede zwischen einem 3B und 7B Modell, weil die Antworten dann knapper und einfacher Ausfallen. Wegen dem Problem habe ich dann erstmal alle Bilder mit llava-phi3 von Microsoft, was ein recht kleines Modell ist, aber eigentlich ganz Passable antworten liefert

### Was nun?
Ganz einfach: Ab in die Shell (in Windows heißt es Terminal) und z.B. das erste Modell mittels `ollama pull llava-phi3` runterladen. Mit `ollama run llava-phi3` ließe sich dann in der Shell das Modell starten und eingaben Tätigen. Das ist aber für uns vielleicht nicht ganz so praktisch, wenn man in die Shell jedes Foto einzeln reinkopieren müsste. Netterweise bietet Ollama auch direkt eine [Web-API](https://github.com/ollama/ollama/blob/main/docs/api.md) an, die sich recht einfach ansprechen lässt. Aber man kann natürlich auch einfach wie ich das entsprehcende Ollama Python Package nutzen :). 

## Die Prompts
Ich habe die Prompts relativ einfach gehalten und je nach verwendeten Sprachmodell lohnt es sich diese auch Anzupassen. Während phi3 sehr knappe Antworten liefert und diese auch bei geänderten Prompts sehr ähnlich ausfallen, sieht es bei llama3 komplett anders aus und man bekommt entweder z.B. einen Titel mit 3 Wörtern oder eines aus 3 Sätzen.
...
Den Code sehr ihr am Ende von diesem Beitrag. Aber im Prinzip funktioniert der Code folgendermaßen: 

1. Scan den Pfad zu den Fotos und erstell eine Liste mit allen jpeg, jpg und png Dateien.
2. Schleife durch alle Bilder.
2.1 Starte das Sprachmodel.
2.2 Konvertiere das Bild in Base64.
2.3 Übergib das Bild mit Prompt dem Sprachmodel.
...
## Der Code
Es müssen eigentlich nur die Konstanten angepasst werden. OLLAMA_URL muss nicht angerührt werden, aber wenn man mehrere Modelle parallel laufen lässt, dann macht es Sinn zumindest den Port abzuändern damit es nicht zu Konflikten kommt. 
