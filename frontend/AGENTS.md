...
- **Stale-chunk recovery.** After a deploy, a tab running an older build can request chunks that no longer exist. Recovery happens in two places. The inline `<head>` listener from `src/lib/staleChunkReload.ts` catches failed initial script loads and chunk-load rejections before hydration. `app/error.tsx` and `app/global-error.tsx` handle failures that reach an error boundary. Both reload the page at most once every 30 s and render an empty placeholder in place of the error UI while the reload is pending. Error signatures are listed in `STALE_CHUNK_ERROR_SIGNATURES` (`src/lib/errors.ts`), including Turbopack's `Failed to load chunk …` / `name === 'ChunkLoadError'`. Boundary errors are reported to Umami as `client-error` events. The listener is serialized with `Function.prototype.toString()`, so keep it self-contained: no imports or outer-scope references.
